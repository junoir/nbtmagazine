...
#-------------------------------------------  
    public function website_timezone() {
        ...
        if ($settings != false) {
            $setting_details = $this->objectToArray(json_decode('[' . $settings . ']'));
            $data .= 'date_default_timezone_set(' . "'" . $setting_details[0]['website_timezone'] . "');\n\n";
            $data .= '$time_zone = ' . "'" . $setting_details[0]['website_timezone'] . "';\n\n";
            return $data;
        } else {
            return '';
        }
...
#-------------------------------------------
    public function get_nenu_by_position() {
        $query = $this->db->query("SELECT slug,position,category_name FROM categories WHERE menu='1' ORDER BY position ASC");
        $num_rows = $query->num_rows();
        if ($num_rows > 0) {
            return $query->result();
        } else {
